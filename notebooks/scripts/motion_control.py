...
class motion_object:
	...
	def __init__(self, all_param_vector, learned_indices):				

		self.initialize() # reset the audio object (socket)

		# Set the motion parameters which are being learned
		# Each parameter ranges on an INTEGER value from 0 to 4 value --> 0 is lowest, 4 is highest
		self.learn_param_0 = all_param_vector[learned_indices[0]]
		self.learn_param_1 = all_param_vector[learned_indices[1]]
		self.learn_param_2 = all_param_vector[learned_indices[2]]

	# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

	def initialize(self, load_file=None):
		""" Initialize the counter n to either 0 or 1 - dependant. 
			The algorithm follows the Q-table if we initialize with a known action-value dataset
			Setting each action's self.n to 1 at the beginning means we will not receive "inf" for 
			each uncertainty, and only have to try actions with pre-initialized high value """
	...
	def probe(self, all_states):
		""" have robot generate motion gesture then return a reward based on the answer provided by the user. """
			...
			if state_probe_complete == False:
				while True:
					try:
						print("------------------------------------------------------------------------")
						print("------------------------------------------------------------------------\n")
						print("Robot is taking a pose....\n")
						cprint(f"What state is the robot in: \n", "black", "on_green", attrs=["bold"])
						print(f"[S]: {all_states[0]} \n[A]: {all_states[1]} \n[P]: {all_states[2]} \n[N]: {all_states[3]} \n\n")
						print("To have robot execute pose again: leave the input empty and hit 'enter'...")
						...
						# code to make robot execute motion again
						continue
		...
		# User has now responded --> stop the robot (lay down or go to force stand)
	...
	def execute_motion(self):
		""" execute the robot motion based on the arguments. Here use FreeDog """	 

		cprint(f"Robot sound playing for:\nParam 0: {self.learn_param_0} \nParam 1: {self.learn_param_1} \nParam 2: {self.learn_param_2}\n", "black", "on_green", attrs=["bold"])
